...
public class Parser {
	 ...
	 * Create a scene.
	 */
	public static Scene createScene(Node dataNode)	// The main method that starts scene creation
													// Called from A5App.run() with document.getDocumentElement() parameter
	{
		Scene scene = new Scene();
		
		// Scene is the basic node of the XML file with ambient color of the scene

		// Most basic scene encapsulates LIGHT, MATERIALs of the elements in the scene,
		// Render configuration (background color, output file name, camera attributes etc.),
		// and scene objects
		
		// Ambient color
		Node ambientAttr = dataNode.getAttributes().getNamedItem("ambient");	// Read ambient color of the scene
		
        if( ambientAttr != null )	// Read ambient color values and set
        {
        ...
        // Read child-nodes of scene node
        ...
        for ( int i = 0; i < nodeList.getLength(); i++ )
        {
            ...
            String nodeName = n.getNodeName();	// Read next node
            
            if ( nodeName.equalsIgnoreCase( "material" ) )	// name, diffuse, specular, hardness
            {                
                Material material = Parser.createMaterial(n);
                Material.materials.put( material.name, material );
            }
            else if( nodeName.equalsIgnoreCase( "light" ) )	// name, color, position ("from"), power(?), type (Point, direction etc.)
            {                
                Light light = Parser.createLight(n);
                scene.lights.put( light.name, light);
            }
            else if ( nodeName.equalsIgnoreCase( "render" ) )	// background color, output, samples (Probably the sampler)	// TODO ??? Sampler
            {                
                scene.render = Parser.createRender(n);	// Has a child node (camera -> name, position ("from"), to, up, fov, and image width, height
            }
            else if ( nodeName.equalsIgnoreCase( "node" ) )	// Generates objects
            {
...
//	}
	 ...
	 */
	public static SceneNode createSceneNode(Node dataNode)	// Generates scene objects
	{
		// Generate a new SceneNode object, and set its name
		SceneNode sceneNode = new SceneNode();
        sceneNode.name = dataNode.getAttributes().getNamedItem("name").getNodeValue();	
        
        sceneNode.id = 666; //idCounter;
		//idCounter++;
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
			...
			Node imageAttr = dataNode.getAttributes().getNamedItem("texture");
			
			if ( imageAttr != null )
			{
				try
				{
					material.img = ImageIO.read(new File(imageAttr.getNodeValue()));
	...
	public static Mesh createMesh(Node dataNode)
	{
        ...
        if ( filenameAttr != null )
        {
        	...
        	mesh.bbox.min.x = mesh.soup.min.x;
        	mesh.bbox.min.y = mesh.soup.min.y;
        	mesh.bbox.min.z = mesh.soup.min.z;

        	mesh.bbox.max.x = mesh.soup.max.x;
        	mesh.bbox.max.y = mesh.soup.max.y;
        	mesh.bbox.max.z = mesh.soup.max.z;
